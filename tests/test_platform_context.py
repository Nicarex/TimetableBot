...
def test_resolve_email():
    ...
    assert ctx == PlatformContext('email', 'email', 'email', 'test@mail.com')


def test_resolve_vk_chat():
    ctx = resolve_platform(vk_id_chat='12345')
    assert ctx == PlatformContext('vk_chat', 'vk_chat', 'vk_id', '12345')


def test_resolve_vk_user():
    ctx = resolve_platform(vk_id_user='67890')
    assert ctx == PlatformContext('vk_user', 'vk_user', 'vk_id', '67890')


def test_resolve_telegram():
    ctx = resolve_platform(telegram='111222')
    assert ctx == PlatformContext('telegram', 'telegram', 'telegram_id', '111222')


def test_resolve_discord():
    ctx = resolve_platform(discord='333444')
    assert ctx == PlatformContext('discord', 'discord', 'discord_id', '333444')
